...
function LessonCard({ lesson }: { lesson: LessonWithProgress }) {
  ...
  const isDone = lesson.completed_at !== null;
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        ...
        {isDone ? (
          <p className="text-[11px] font-semibold" style={{ color: isPerfect ? '#16A34A' : '#16A34A' }}>
            {isPerfect ? '⭐ Perfect · ' : '✓ '}{formatDate(lesson.completed_at)}
          </p>
        ) : (
          <p className="text-[11px] font-semibold" style={{ color: '#D97B6C' }}>
            Not started
          </p>
        )}
      </div>
      ...
      <div
        ...
        style={{ background: isDone ? '#16A34A' : '#D97B6C' }}
      />
    </Link>
  );
}
...
export default function GrammarPage() {
  ...
  const activeLessons = lessons.filter(l => l.level === activeLevel);
  const completed = activeLessons.filter(l => l.completed_at).length;
    ...
    <div className="max-w-3xl mx-auto px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-3">
        <span
          ...
          style={{ background: LEVEL_COLORS[activeLevel].bg, color: LEVEL_COLORS[activeLevel].text }}
        >
          {activeLevel}
        </span>
        <span className="text-xs text-muted font-semibold">{completed}/{activeLessons.length} completed</span>
      </div>

      {/* Lesson grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {activeLessons.map(lesson => <LessonCard key={lesson.id} lesson={lesson} />)}
      </div>
    </div>
  );
}

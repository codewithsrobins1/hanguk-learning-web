...
export default function ShadowSessionPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-cream" style={{ fontFamily: 'system-ui, sans-serif' }}>
      {/* Header */}
      <div className="flex items-center justify-between gap-3 px-4 py-3" style={{ background: '#1A1F36' }}>
        <button
          onClick={() => router.back()}
        ...
        <button
          ...
          style={{
            ...
            border: showTranslations ? '1.5px solid #E8412C' : '1.5px solid rgba(255,255,255,0.18)',
            color: '#F7F4EE',
          }}
        >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-2 pb-32">
        {dialogue.lines.map((l, i) => {
          ...
          return (
            <div key={i} className="flex gap-2" style={{ flexDirection: isA ? 'row' : 'row-reverse', opacity: isPast ? 0.4 : 1, transition: 'opacity 0.2s' }}>
              <div className="flex items-center justify-center flex-shrink-0" style={{ width: 30, height: 30, borderRadius: '50%', background: isA ? '#1A1F36' : '#E8412C', marginTop: 3 }}>
              ...
              >
                <p style={{ margin: 0, fontSize: 19, fontWeight: 600, color: isActive ? '#F7F4EE' : '#111', fontFamily: 'Noto Sans KR, sans-serif', lineHeight: 1.5 }}>
                  {l.korean}
                </p>
                {showTranslations && (
                  <p style={{ margin: '5px 0 0', fontSize: 15, color: isActive ? 'rgba(255,255,255,0.65)' : '#888', lineHeight: 1.4 }}>
                    {l.translation}
                  </p>
                )}
              </div>
      ...
      {/* Bottom controls */}
      <div className="fixed bottom-16 md:bottom-0 left-0 right-0 bg-white border-t border-border px-5 py-4 flex gap-3 md:ml-56">
        <button
        ...
        {isLast ? (
          <button
            onClick={async () => {
              if (xpEarned === 0 && user) {
                setXpEarned(10);
                await saveDialogueProgress(user.uid, dialogueId);
